...
bool CleanPCA::clean(){
  ...
  dataArray->eVectors.resize(nDetectors,nDetectors);
   ...
   #pragma omp  parallel shared (dataArray, tel, di, nScans, nDetectors, neigToCut, cutStd,cerr,cout)\
				   ...
				   mn,mk, denom, pcaCorr) default (none)
  {
	  ...
	  for(k=0;k<nScans;k++){
		  ...
		  //Subtract the median of each bolometer and copy the data into them
		  for(i=0;i<nDetectors;i++){
			  mn = median(&dataArray->detectors[di[i]].hValues[si], &dataArray->detectors[di[i]].hSampleFlags[si] , ei-si);
			  mk = median(&dataArray->detectors[di[i]].hKernel[si], &dataArray->detectors[di[i]].hSampleFlags[si] , ei-si);
			  for(j=si;j<ei;j++){
		  ...
		  if(neigToCut > 0){
			  ...
			  if (!ap->icaMode){
				  ...
				  cerr<<"PCA ICA MODE!!!"<<endl;
//				  unsigned long bs;
				  for(i=0;i<nDetectors;i++) {
//					  if ( (i!= 0 && i<20)){
//						  bs = 1l<<(i-1);
//						  if (bs & neigToCut){
//							  cout<<" Preserving eigenvector "<< i<<endl;
//							  continue;
//						  }
//					  }
					  if (neigToCut-1 == i)
