...
///WienerFilter constructor
/** This method applies a wiener filter to the signal and weight
    maps using a rotationally symmetric version of the point source
    kernel and an average noise psd.  Throughout this code I
    will try to match the variable names of aztec_wiener_filter.pro.
    /todo - gaussian template, normalize errors
 **/
WienerFilter::WienerFilter(AnalParams* analParams, Coaddition* cmap)
{
	//set our ap pointer
	ap = analParams;
	...
	for(int i=0;i<nrows;i++)
		for(int j=0;j<ncols;j++)
			if(cmap->weight->image[i][j] == 0.) zcount++;
	VecInt whz;
	if(zcount > 0){
		whz.resize(zcount);
		zcount = 0;
	...
	diffx = abs(cmap->getRowCoordsPhys(1)-cmap->getRowCoordsPhys(0));
	diffy = abs(cmap->getColCoordsPhys(1)-cmap->getColCoordsPhys(0));

	//----- o -----
	//prepare the template
	if(ap->getHighpassOnly()){
	  tplate.assign(nx,ny,0.);
	  tplate[0][0]=1.;
	} else if(ap->getGaussianTemplate()){
 ...
 **/
bool WienerFilter::filterCoaddition(Coaddition* cmap){
	...
	for(int i=0;i<nx;i++) for(int j=0;j<ny;j++)
		mflt[i][j] = cmap->kernel->image[i][j];
	calcNumerator(mflt);
	for(int i=0;i<nx;i++)
		for(int j=0;j<ny;j++){
			if (Denom[i][j] != 0.0)
	...
	for(int i=0;i<nx;i++) for(int j=0;j<ny;j++)
		mflt[i][j] = cmap->signal->image[i][j];
...
/** This is the main driver for rapidly filtering the images from
    ...
    calculated.
 **/
bool WienerFilter::filterNoiseMaps(NoiseRealizations *nr){
	...
	for(int k=0;k<nr->nNoiseFiles;k++){
	  ...
	  cerr << "WienerFilter(): Filtering noise map " << k << ".\r";
		...
		MatDoub noise(nrows, ncols);
		NcVar* noisev = ncfid.get_var("noise");
		noisev->get(&noise[0][0], nrows, ncols);

		//do the filtering
		MatDoub filteredNoise(nrows,ncols,0.);
		MatDoub filteredWeight(nrows,ncols,0.);
		calcNumerator(noise);
		for(int i=0;i<nx;i++) for(int j=0;j<ny;j++){
		    if(Denom[i][j] > 0){
...
bool WienerFilter::calcRr(Coaddition *cmap)
{
	...
	if(uniformWeight){
		...
		rr.resize(nx,ny);
		for(int i=0;i<nx;i++) for(int j=0;j<ny;j++)
			rr[i][j] = sqrt(cmap->weight->image[i][j]);
	}
	return 1;
}
 ...
 **/
bool WienerFilter::calcNumerator(MatDoub &mflt)
{
 ...
 **/
bool WienerFilter::calcDenominator()
{
...
bool WienerFilter::prepareTemplate(Coaddition *cmap)
{
  ...
  VecDoub binlow(nbins);
  for(int i=0;i<nbins;i++) binlow[i] = (double) (i*diffx);
  
  VecDoub kone(nbins-1,0.);
  VecDoub done(nbins-1,0.);
...
bool WienerFilter::prepareGaussianTemplate(Coaddition *cmap)
{
...
//	for (int i=0; i<nx; i++)
//		for (int j=0; j<ny; j++){
//			in[i*ny +j][0]= cmap->signal->image[i][j];
//			in[i*ny +j][1]= 0.0;
//		}
//	fftw_execute(pf);
//
...
//	for (int i=0; i<nx; i++)
//		for (int j=0; j<ny; j++){
//			cmap->signal->image[i][j] = out[i*ny+j][0];
//		}
//
//	fftw_free(in);
//	fftw_free(out);
...
void WienerFilter::simpleWienerFilter2d(Coaddition *cmap){

	MatDoub mapPsd = cmap->signal->psd2d;
	...
	MatDoub noisePsd (nx2dpsd, ny2dpsd);
	...
	MatDoub psdFilter (nx2dpsd,ny2dpsd,0.0);
	double sn = 0;
	...
	MatDoub hann = hanning(nx2dpsd,ny2dpsd);

	int mx = cmap->signal->image.nrows();
	int my = cmap->signal->image.ncols();
	...
	for (int i=0; i<nx2dpsd; i++)
		for (int j=0; j<ny2dpsd; j++){
			in[i*ny2dpsd +j][0]= cmap->signal->image[dx+i][dy+j];
	...
	cmap->signal->image.assign(mx,my, 0.0);
	double norm = nx2dpsd*ny2dpsd;
	for (int i=0; i<nx2dpsd; i++)
			for (int j=0; j<ny2dpsd; j++){
				cmap->signal->image[dx+i][dy+j]=in[i*ny2dpsd +j][0] /norm;
			}
