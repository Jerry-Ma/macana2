...
**/
Map::Map(string name, int nr, int nc, double pixsz, MatDoub &wt, VecDoub &rcp,
         VecDoub &ccp) {
  // initialization
  nrows = nr;
  ncols = nc;
  ...
  for (int i = 0; i < nrows; i++)
    rowCoordsPhys[i] = rcp[i];
  for (int j = 0; j < ncols; j++)
    colCoordsPhys[j] = ccp[j];
  weight.resize(wt.nrows(), wt.ncols());
  for (int i = 0; i < wt.nrows(); i++)
    for (int j = 0; j < wt.ncols(); j++)
      weight[i][j] = wt[i][j];
...
**/
double Map::fitToGaussianMasked(VecDoub &pp, VecInt &fixme, VecDoub &fixVals,
                                double *iguess, int deg) {
  ...
  cerr << "fitting to gaussion with # " << npts << " data points" << endl;
  VecDoub params(7);
  VecDoub params_err(7);
  MatInt gi(npts, 2); // keep a record of good indices
  VecDoub az(npts);
  VecDoub el(npts);
  VecDoub m(npts);
  VecDoub sigma(npts);
  // fill the good indices array
  int ngi = 0;
  for (int i = 0; i < nptsrows; ++i)
    for (int j = 0; j < nptscols; ++j) {
      if ((image[i + minxi][j + minyi] != 0.) &&
          (weight[i + minxi][j + minyi] != 0.)) {
        gi[ngi][0] = i;
        gi[ngi][1] = j;
        ++ngi;
      }
    }
  cerr << "# of good data: " << ngi << endl;
  // fill the data
  for (int k = 0; k < ngi; ++k) {
    int i = gi[k][0];
    int j = gi[k][1];
    az[k] = rowCoordsPhys[i + minxi];
    el[k] = colCoordsPhys[j + minyi];
    m[k] = image[i + minxi][j + minyi];
    sigma[k] = sqrt(1. / weight[i + minxi][j + minyi]);

  }
  // call the fitter for the map
  double chisqpdof;
  chisqpdof = mpGaussFit(&params[0], &params_err[0], &az[0], &el[0], &m[0],
                         &sigma[0], ngi, fixme, fixVals, iguess);

  cerr << mapName << " Gaussian fit parameters: " << endl;
  cerr.precision(4);
  cerr << "   dc offset = " << params[0] << " +/- " << params_err[0] << " Jy"
       << endl;
  cerr << "   amplitude = " << params[1] << " +/- " << params_err[1] << " Jy"
       << endl;
  cerr << "   FWHM x   = " << params[2] / TWO_PI * 360. * 3600. * 2.3548
       << " +/- " << params_err[2] / TWO_PI * 360. * 3600. * 2.3548 << " arcsec"
       << endl;
  cerr << "   FWHM y   = " << params[3] / TWO_PI * 360. * 3600. * 2.3548
       << " +/- " << params_err[3] / TWO_PI * 360. * 3600. * 2.3548 << " arcsec"
       << endl;
  cerr << "   offset x  = " << params[4] / TWO_PI * 360. * 3600. << " +/- "
       << params_err[4] / TWO_PI * 360. * 3600. << " arcsec" << endl;
  cerr << "   offset y  = " << params[5] / TWO_PI * 360. * 3600. << " +/- "
       << params_err[5] / TWO_PI * 360. * 3600. << " arcsec" << endl;
...
double Map::fitToGaussian(VecDoub &pp, VecInt &fixme, VecDoub &fixVals,
                          double *iguess, int deg) {
  // if map is big, make assumption that gaussian is within
  // 40" of center of map, otherwise skip this
  double rowsize = rowCoordsPhys.size() * pixelSize;
  ...
  for (int i = 0; i < nptsrows; i++)
    for (int j = 0; j < nptscols; j++) {
      ...
      //      (image[i+minxi][j+minyi] * 0.01);
            ...
            sigma[nptscols*i + j] = (weight[i+minxi][j+minyi] == 0.) ? 1. : sqrt(1./weight[i+minxi][j+minyi]);
//      sigma[nptscols * i + j] = (weight[i + minxi][j + minyi] == 0.)
 //                                   ? nan("1")
  //                                  : weight[i + minxi][j + minyi];
    }

  // call the fitter for the map
  double chisqpdof;
  chisqpdof = mpGaussFit(&params[0], &params_err[0], &az[0], &el[0], &m[0],
                         &sigma[0], npts, fixme, fixVals, iguess);
...
double Map::fitToGaussian(VecDoub &pp, int deg) {
  VecInt fixme(7, 0);
  VecDoub fixVals(7, 0.);
  return fitToGaussianMasked(pp, fixme, fixVals, nullptr, deg);
}
...
bool Map::findWeightThresh() {
  ...
  // MVALI IS NOW THE INDEX THAT IS HALFWAY BEWEEN THE NUMBER OF POINTS AND
  // COVLIMI, THE 75% INDEX
...
int Map::getNrows() { return nrows; }

int Map::getNcols() { return ncols; }

int Map::getNPixels() {return nPixels;}

double Map::getPixelSize() { return pixelSize; }

double Map::getRowCoordsPhys(int i) { return rowCoordsPhys[i]; }

double Map::getColCoordsPhys(int i) { return colCoordsPhys[i]; }

double Map::getCoverageCut() { return coverageCut; }
...
int Map::getCutXRangeLow() { return cutXRange[0]; }

int Map::getCutXRangeHigh() { return cutXRange[1]; }

int Map::getCutYRangeLow() { return cutYRange[0]; }

int Map::getCutYRangeHigh() { return cutYRange[1]; }
